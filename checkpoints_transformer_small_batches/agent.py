...
def create_agent(env: CinchMainEnv) -> CinchTransformer:
    """
    Creates a new instance of the CinchTransformer with the specified input size.

    Args:
        input_size: The size of the input tensor (observation space).
    
    Returns:
        An instance of the CinchTransformer.
    """
    return CinchTransformer(
        d_model=96,
        nhead=4,
        num_layers=3,
        dim_feedforward=256
...
def select_action(model: CinchTransformer, obs: dict, env: CinchMainEnv, DEVICE: torch.device) -> int:
    """
    Selects an action using the model given the current observation and environment.
    
    Args:
        model: The CinchTransformer model to use for action selection.
        obs: The current observation from the environment.
        env: The environment, used to get legal actions.
